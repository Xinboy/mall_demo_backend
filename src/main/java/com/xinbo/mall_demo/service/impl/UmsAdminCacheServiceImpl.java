...
public class UmsAdminCacheServiceImpl implements UmsAdminCacheService {
    ...
    public void delResourceListByRole(Long roleId) {
        ...
        if (CollUtil.isNotEmpty(relations)) {
            ...
            List<String> keys = relations.stream().map(relation -> keyPrefix + relation.getAdminId()).collect(Collectors.toList());
    ...
    public void delResourceListByResource(Long resourceId) {
        ...
        if (CollUtil.isNotEmpty(adminIds)) {
            String keyPrefix = REDIS_DATABASE + ":" + REDIS_KEY_RESOURCE_LIST + ":";
            List<String> keys = adminIds.stream().map(adminId -> keyPrefix + adminId).collect(Collectors.toList());
            redisService.del(keys);
        }
    }
